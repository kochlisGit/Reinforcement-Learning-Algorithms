...
class Agent:
    def __init__(self, epsilon, gamma, alpha, lambda_):
        self.epsilon = epsilon
        self.gamma = gamma
        self.alpha = alpha
        self.lambda_ = lambda_
        self.Q = np.zeros( shape=(num_of_states, num_of_actions), dtype=np.float32 )
        self.E = np.zeros( shape=(num_of_states, num_of_actions), dtype=np.float32 )
        self.state_history = []
    ...
    # Prints the Eligibility Traces Function
    def print_E_traces(self):
        print(self.E)
        ...
        if p < self.epsilon:
            ...
            for action in range(4):
                if action in possible_actions and self.Q[state][action] > highest_reward:
                    ...
                    next_action = action
        return next_action, is_random

    
    def update(self, reward, action, is_random, new_state):
...
def train(epsilon=0.1, gamma=0.3, alpha=0.7, lambda_=0.75, test_agent=False):
    agent = Agent(epsilon, gamma, alpha, lambda_)
    num_of_episodes = 50
